...
@ray.remote
def DirLinProdKDE_Fast(x, data, h=[None,None], com_type=['Dir', 'Lin'], dim=[2,1]):
    '''
...
@ray.remote
def DirLinProdMS_Fast(mesh_0, data, h=[None,None], com_type=['Dir','Lin'], dim=[2,1], 
    ...
    Return:
        MS_new: (m, sum(dim)+sum(com_type=='Dir'))-array
            The collection of converged points yielded by the mean shift algorithm.
    '''
        ...
        for i in range(mesh_0.shape[0]):
            if conv_sign[i] == 0:
                for k in range(len(dim)):
                    ...
                    x_comp = MS_old[i,sum(Eu_dim[:(k+1)]):sum(Eu_dim[:(k+2)])]
                    # Kernel weights
                ...
                for k in range(len(dim)):
                    if com_type[k] == 'Dir':
                        ...
                        MS_new[i,sum(Eu_dim[:(k+1)]):sum(Eu_dim[:(k+2)])] = x_Dir_new
                    elif com_type[k] == 'Lin':
                        data_comp = data[:,sum(Eu_dim[:(k+1)]):sum(Eu_dim[:(k+2)])]
                        x_Lin_new = np.sum(data_comp * ker_w_mul, axis=0) / np.sum(ker_w_mul)
                        MS_new[i,sum(Eu_dim[:(k+1)]):sum(Eu_dim[:(k+2)])] = x_Lin_new
                # Check if the current point is converged
                if LA.norm(MS_new[i,:] - MS_old[i,:]) <= eps:
                    conv_sign[i] = 1
            else:
                MS_new[i,:] = MS_old[i,:]
        # print(t)
...
@ray.remote
def DirLinProdMSCompAsc_Fast(mesh_0, data, h=[None,None], com_type=['Dir','Lin'], 
    ...
    for i in range(mesh_0.shape[0]):
        for k in range(len(dim)):
            data_comp = data[:,sum(Eu_dim[:(k+1)]):sum(Eu_dim[:(k+2)])]
            x_comp = MS_new[i,sum(Eu_dim[:(k+1)]):sum(Eu_dim[:(k+2)])]
            # Kernel weights
...
@ray.remote
def DirLinProdSCMSLog_Fast(mesh_0, data, d=1, h=[None,None], com_type=['Dir','Lin'], 
    ...
    Return:
        SCMS_new: (m, sum(dim)+sum(com_type=='Dir'))-array
            The collection of converged points yielded by our proposed SCMS 
            algorithm.
    '''
    ...
    for t in range(1, max_iter):
        ...
        for i in range(mesh_0.shape[0]):
            if conv_sign[i] == 0:
                ...
                for k in range(len(dim)):
                    data_comp = data[:,sum(Eu_dim[:(k+1)]):sum(Eu_dim[:(k+2)])]
                    x_comp = SCMS_old[i,sum(Eu_dim[:(k+1)]):sum(Eu_dim[:(k+2)])]
                    # Kernel weights
                    ker_w_comp = np.exp(-np.sum(((x_comp - data_comp)/h[k])**2, axis=1)/2)
                    if k == 0:
                        ker_w_mul = ker_w_comp.reshape(n,1)
...
@ray.remote
def DirLinProdSCMSLog_Naive_Fast(mesh_0, data, d=1, h=[None,None], com_type=['Dir','Lin'], 
                   dim=[2,1], eps=1e-7, max_iter=1000):
    '''
    ...
    Return:
        SCMS_new: (m, sum(dim)+sum(com_type=='Dir'))-array
            The collection of converged points yielded by the naive SCMS 
            algorithm.
    '''
